...
import { FIELD_LENGTH, FIELD_WIDTH, GOAL_WIDTH, FIELD_HALF_WIDTH } from '../constants.js'
import type {
  FramePosterior,
  BeliefDelta,
  OutcomeDistribution,
...
): { xG: number; bucket: keyof OutcomeDistribution } {
  const lastFrame = simResult.frames[simResult.frames.length - 1]
  if (!lastFrame) return { xG: 0, bucket: 'retention' }

  const ballPos = lastFrame.positions['ball']
  if (!ballPos) return { xG: 0, bucket: 'retention' }
...
function klDivergence(p: OutcomeDistribution, q: OutcomeDistribution): number {
  const keys: (keyof OutcomeDistribution)[] = ['turnover', 'retention', 'progression', 'opportunity', 'goal']
  const eps = 1e-6
  let kl = 0
  for (const k of keys) {
